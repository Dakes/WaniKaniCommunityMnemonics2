...
${isItem ?
    ...
    function getItem() {
        ...
        if (isItem) {
            ...
            item = $$1.jStorage.get("currentItem")["characters"];
            // image radical case, two methods, if one breaks
            if (getShortItemType(getItemType()) === "r" && item == null) {
                let jstorageEn = $$1.jStorage.get("currentItem")["en"];
                if (jstorageEn != null)
            ...
            item = $$1.jStorage.get("l/currentLesson")["characters"];
            // image radical case
            if (getShortItemType(getItemType()) === "r" && item == null) {
                let jstorageEn = $$1.jStorage.get("l/currentLesson")["en"];
                if (jstorageEn != null)
                    item = jstorageEn[0].toLowerCase();
            }
            if (getShortItemType(getItemType()) === "r" && item == null) {
     ...
     * */
    function getItemType( /*short: boolean=true*/) {
        let itemType = null;
        if (isReview)
            itemType = $$1.jStorage.get("currentItem")["type"];
        else if (isLesson)
            itemType = $$1.jStorage.get("l/currentLesson")["type"];
        else if (isItem) {
            itemType = window.location.pathname.slice(window.location.pathname.indexOf("com/") + 2, window.location.pathname.lastIndexOf("/"));
...
<button type="button" id="cm-${mnemType}-cancel" class="cm-btn cm-cancel-highlight disabled:cursor-not-allowed disabled:opacity-50">Cancel</button></div>
    ...
    class Buttons {
         ...
         * */
        static toggleVotes(mnemType, scoreJson, mnemUser, mnemIndex) {
            try {
     ...
     * */
    async function getData(item, type) {
        if (type == undefined)
    ...
    async function voteMnemonic(mnemType, item, shortType, vote) {
        let shortMnemType = getShortMnemType(mnemType);
        let url = sheetApiUrl +
            `?exec=vote&item=${item}&type=${shortType}&mnemType=${shortMnemType}&user=${WKUser}&mnemUser=${currentMnem.currentUser[mnemType]}&mnemIndex=${currentMnem.userIndex}&score=${vote}`;
        url = encodeURI(url);
        return fetch(url, { method: "POST" });
    }
